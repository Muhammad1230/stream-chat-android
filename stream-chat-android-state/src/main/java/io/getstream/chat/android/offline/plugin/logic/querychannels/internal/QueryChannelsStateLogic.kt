...
internal class QueryChannelsStateLogic(
    ...
    internal fun addChannelsState(channels: List<Channel>) {
        ...
        channels.forEach { channel ->
            logicRegistry.channel(channel.type, channel.id).updateDataFromChannel(
                channel,
    ...
    internal fun refreshChannels(cidList: Collection<String>) {
        logger.d { "refreshing channels with cid: ${cidList.joinToString()}" }
        ...
        logger.d { "channels that will be updated: ${newChannels.values.joinToString { it.name }}" }
        logger.d { "mutableState: $mutableState" }
        logger.d { "updating channelsStateData: ${mutableState.channelsStateData}" }

        mutableState.rawChannels = newChannels
    }
